...
const Contact = () => {
  ...
  return (
    <section id="contact" className="min-h-screen flex items-center justify-center py-10 overflow-hidden relative">
      {/* Background Decor */}
      ...
      <div className="max-w-7xl w-full px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-start">
          ...
          >
            <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-8 shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-shadow">
            ...
            {/* Decor Card */}
            <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden transform transition-transform hover:-translate-y-1 duration-300">
          ...
          <motion.div
            ...
            className="bg-white rounded-[2rem] p-8 md:p-10 shadow-xl shadow-slate-200/50 border border-slate-100 relative"
          >
            {success && (
              <motion.div
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 0 }}
                className="absolute top-0 left-0 w-full bg-green-500 text-white text-center py-2 rounded-t-[2rem] font-medium text-sm"
              >
                {success}
              </motion.div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6 mt-2">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-slate-900 text-white py-4 rounded-xl font-semibold hover:bg-indigo-600 transition-colors duration-300 flex items-center justify-center gap-2 group shadow-lg shadow-slate-900/20 disabled:opacity-70 disabled:cursor-not-allowed"
              >
...
const ContactItem = ({ icon, label, value, href }) => (
  <div className="flex items-center gap-4 group cursor-pointer">
    ...
    <div className="flex-1">
      ...
      {href ? (
        <a href={href} target="_blank" rel="noopener noreferrer" className="text-slate-900 font-semibold group-hover:text-indigo-600 transition-colors inline-flex items-center gap-1">
          {value}
